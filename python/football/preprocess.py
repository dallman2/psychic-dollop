...
def extract_data_from_html(raw, home: str) -> list:
    ...
    # determine whether the home team stars with the ball

    print(rows[0])
    print(rows[1])
    text = rows[1].get_text()
    print(text.split(','))
...
def _standardizer(home: str, q: str, time: str, down: str, dist: str, los: str, away_score: str, home_score: str, desc: str, exp_b: str, exp_a: str, poss_change: bool, first: bool) -> list:
    # this allows the function to be called with only the args poss_change and first to set up the possesion tracking
    global home_poss
    if first:
        home_poss = poss_change
        return
    if poss_change: 
        home_poss = not home_poss
    ...
    return [
        ...
        float(exp_b), # expected value before play
        float(exp_a), # expected value after play
        bool(home_poss), # does the home team possess the ball on this play?
        str(desc), # description of the play
    ]
