<template>
  ...
  >
    ...
    <StoryPoint :wide="wide" icon="rocket_launch" :left="true">
      <template #label> It all started with a fun, family game... </template>
      <template #content>
        Many years ago, 12 to be exact, my dad started putting together a weekly
        NFL pick 'em for the whole family to complete. Since then, we have
        completed the Football Pool, as we call it, every single week of the NFL
        regular season.
      </template>
    </StoryPoint>
    <StoryPoint :wide="wide" icon="assignment">
      ...
      <template #content>
        The game is simple, every week, you have to pick a winner for each NFL
        game based on the money line (pick who wins each game, regardless of the
        spread). Whoever has the most picks correct every week is the winner!
        Ties are broken by also predicting the total number of points scored in
        the Pittsburgh Steeler's game that week.
      </template>
    </StoryPoint>
    ...
    <StoryPoint :wide="wide" icon="online_prediction">
      ...
      <template #content>
        What if I could build a system that predicts the outcome of a football
        game? What about a system that could grade my picks after each week?
        Maybe such a system could help me improve picks that I am only
    ...
    <StoryPoint :wide="wide" icon="data_thresholding" :left="true">
      ...
      <template #content>
        ...
        outcome of something as competative as the NFL <b>is</b> very hard. In
        my opinion, games that have already occurred do not have as much of in
        impact on the outcome of a given game as the plays that have occurred in
        that game. That is to say, if a team is struggling on during a given
        game, that is a better predictor of the outcome than their record coming
        into that game.
      </template>
    </StoryPoint>
    ...
    <StoryPoint :wide="wide" icon="question_mark" :left="true">
      <template #label> How is this useful for the Football Pool? </template>
      <template #content>
        While the system I have described above might not tell me exactly who
        will win ahead of time, it could still grade teams week to week on how
        realiable they are relative to the points
        <span class="text-info"
          >spread.
          <q-tooltip class="text-body2" max-width="25vw">
            The spread is the number of points statisticians beleive will be the
            difference between the final scores
          </q-tooltip>
        </span>
        With that data, I could better predict the outcomes of games before they
        happen, or maybe build another system to do it for me...
      </template>
    </StoryPoint>
